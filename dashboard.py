...
function siteOf(l){ return l.site || ""; }   // link comes from the data/CSV; not editable in the UI
function siteCell(l){
  const v=siteOf(l);
  return v?`<a class="open" href="${esc(v)}" target="_blank" rel="noopener">Open &#8599;</a>`:"&mdash;";
}
...
function rowHtml(l){
  ...
  return `<tr class="${closed?'closed':''}" data-id="${esc(l.id)}">
    ...
    <td class="hide-sm">${siteCell(l)}</td>
    <td>${map}</td>
  </tr>`;
}
function cardHtml(l){
